...
#
# 可用环境变量覆盖:RESP_LEN / ROLLOUT_BSZ / N_SAMPLES / OVERSAMPLE / MAX_TOK / NUM_ENV
# ============================================================================
set -euo pipefail
...
ROLLOUT_BSZ=${ROLLOUT_BSZ:-16}        # 官方 128
...
echo "负载: RESP_LEN=${RESP_LEN} ROLLOUT_BSZ=${ROLLOUT_BSZ} N_SAMPLES=${N_SAMPLES} OVERSAMPLE=${OVERSAMPLE} MAX_TOK=${MAX_TOK}"
...
echo "== 核对改动 =="
grep -nE "num-gpus|gpus-per-node|context-parallel-size|rollout-max-response-len|rollout-batch-size|n-samples-per-prompt|over-sampling-batch-size|max-tokens-per-gpu" "$RLVE_SH"
