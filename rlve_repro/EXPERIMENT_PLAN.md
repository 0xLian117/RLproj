...
Energy** 选难度:`G_e(d) = λ_pref·(−log U_K(p̂)) − λ_info·I(s;o|d) + λ_cost·C(d)`,
`π(d|e) ∝ exp(−G_e(d)/T)`,`U_K(p)=1−p^K−(1−p)^K`。实用项是偏好风险 `−log U_K`
(U_K→0 时→+∞,排斥退化难度)。每环境维护能力后验 `q(s_e)`,
...
| **Signal-RLVE** | 只 `−log U_K`(λ_info=0) | `ARM=signal NUM_ENV=16 bash run_arm.sh RLVE` | 对准 0.5 信号带,但**无信息增益** |
| **FEP-RLVE(ours)** | `−log U_K`+信息增益+softmax | `ARM=fep NUM_ENV=16 bash run_arm.sh RLVE` | 完整 EFE |
